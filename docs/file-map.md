...
| `crates/etv-station/src/constrain.rs` | Post-order adjacency pass (#73). `order_no_repeat` enforces `no_repeat_within = N` — the same `entry_id` may not recur within N positions — over the channel's concatenated list, reaching back across the generation seam via the ledger's `tail`. Deterministic greedy (defer a violator, take the first eligible item behind it) plus a swap-repair; when nothing improves, the violation is accepted so generation always terminates. The list is linear, not circular: `Sequential` plays it once, so its own head and tail never air adjacently. |
