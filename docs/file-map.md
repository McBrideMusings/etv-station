...
| `crates/etv-station/src/config/` | TOML config parsing (Phase C block/channel/entries schema): `station`, `channel`, `rule` (`[[rule.blocks]]` includes), `block` (`[program]`/`duplicates`/`[[entries]]`), `entry` (kind-tagged item/query/include), `pool` (#72 `pools` + `pattern`: `select`/`rotate`/`advance`/`wrap`/`on_short`/`chance`), `source`, `mode`, `order`, `filter`, `load`, `validate`. |
...
| `crates/etv-station/src/resume.rs` | `.resume` sidecar (#72) — the only scheduling state a pattern channel persists: per pool, the last-played `entry_id` per series plus which series is next. Read at generation start, written at the window seam. A missing, corrupt, or future-version file starts every pool from the top rather than failing the channel. |
| `crates/etv-station/src/pattern.rs` | Pattern interleave engine (#72) — resolves each pool, groups it into series by catalog `show_id` (an item without one is its own series), then walks the `{pool, take}` template drawing per `select`/`rotate`/`on_short`/`wrap`. Seeded keyed rolls (SplitMix64) make `select = "random"` and a step's `chance` reproducible for a pinned seed. |
...
| `crates/etv-station/src/rule.rs` | `Rule` trait + two sequencers. `LoopForever` loops the resolver's `ResolvedItem` list — `(t - anchor) mod total_loop_duration` walked over cumulative item durations — to fill the chunk window. `Sequential` (#72) plays a list **once** from a given start, for a pattern channel whose list advances every generation and so materializes forward instead of looping. |
...
| `crates/etv-station/tests/lotr_theatrical_sample.rs` | Integration acceptance test for Sample S3 (#77): resolves `examples/channels/lotr-theatrical.yaml` against a fixture catalog and asserts only theatrical LOTR cuts (edition NULL) resolve, oldest-first, with Extended Editions excluded — proving the `item.edition` filter + the NULL-as-default `!=` rule (#103) keeps NULL-edition films. |
...
| `examples/channels/lotr-theatrical.yaml` | yes | Sample S3 (#77): the S2 LOTR query minus Extended Editions via `item.edition != "Extended Edition"` (theatrical = NULL edition, kept by the NULL-as-default rule #103). Not in `station.yaml`; proven by the `lotr_theatrical_sample` test. |
