...
| `examples/samples/trending-mix.yaml` | yes | Sample S7 (#81): the deepest interleave sample — `pools` + `pattern` weaving 1 trending movie then 3 episodes of a trending show, repeated. Two pools split one "Trending" collection by `type`; `advance: resume` + `select: round_robin` + `wrap: loop` let shows of different lengths each progress independently across window seams. Proven by the `trending_mix_sample` test. |
