...
| `crates/etv-station/src/catalog/order.rs` | Order resolution engine (#69). `Catalog::resolve_order` applies a `config::Order` to a resolved `entry_id` set: `field:dir` compound sorts via SQL `ORDER BY` (nulls last per term, implicit `entry_id` tiebreaker), `manual` keeps authored order, `random` is a SplitMix64-seeded Fisher–Yates shuffle (deterministic per seed). Every case is a function of the ids alone — a collection's authored sequence is deliberately *not* an order (#107), since `collection_items.position` belongs to the (collection, item) pair and a flat id set can't say which collection to read; it is emitted already-ordered by a `kind = "collection"` entry via `Catalog::collection_members`. Non-sortable fields and `score` are errors. |
