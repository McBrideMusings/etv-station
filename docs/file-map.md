...
| `crates/etv-station/src/config/` | TOML config parsing (Phase C block/channel/entries schema): `station`, `channel`, `rule` (`[[rule.blocks]]` includes), `block` (`[program]`/`duplicates`/`[constraints]`/`[[entries]]`), `constraints` (`no_repeat_within`), `entry` (kind-tagged item/query/include), `source`, `mode`, `order`, `filter`, `load`, `validate`. |
...
| `crates/etv-station/src/constrain.rs` | Post-order adjacency pass (#73). `order_no_repeat` enforces `no_repeat_within = N` — the same `entry_id` may not recur within N positions — over the channel's concatenated list, treated as **circular** because `LoopForever` replays it end to end. Deterministic greedy (defer a violator, take the first eligible item behind it) plus a swap-repair for wrap clashes the greedy cannot see coming; when nothing improves, the violation is accepted so generation always terminates. |
