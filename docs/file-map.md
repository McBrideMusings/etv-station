...
| `examples/channels/diehard.toml` | no | Personal Die Hard channel; gitignored. Demonstrates the path form — composes `../blocks/diehard.yaml`. Wired to the Pierce overlay. |
| `examples/blocks/diehard.yaml` | no | Reusable block file (the Die Hard item) referenced by `diehard.toml`; demonstrates the `block = "path"` include form. |
