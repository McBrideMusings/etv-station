...
| `crates/etv-station/src/resume.rs` | `.resume` sidecar (#72) — the only scheduling state a pattern channel persists: per pool, the last-played `entry_id` per series plus which series is next. Read at generation start, written at the window seam. Also holds the checkpoints — pool state entering each unaired generation — that let a restart rewind, drop its unaired chunks, and regenerate them from the current config. A missing, corrupt, or future-version file starts every pool from the top rather than failing the channel. |
