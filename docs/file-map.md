...
| `crates/etv-station/src/config/` | TOML config parsing (Phase C block/channel/entries schema): `station`, `channel`, `rule` (`[[rule.blocks]]` includes), `block` (`[program]`/`duplicates`/`[constraints]`/`[[entries]]`), `constraints` (`no_repeat_within`), `entry` (kind-tagged item/query/collection/include), `pool` (#72 `pools` + `pattern`: `select`/`rotate`/`advance`/`on_short`/`chance`), `source`, `mode`, `order`, `filter`, `load`, `validate`. |
...
| `crates/etv-station/src/constrain.rs` | Post-order adjacency pass (#73). `order_no_repeat` enforces `no_repeat_within = N` — the same `entry_id` may not recur within N positions — over the channel's concatenated list. Deterministic greedy (defer a violator, take the first eligible item behind it) plus a swap-repair; when nothing improves, the violation is accepted so generation always terminates. |
