...
| `window_days` | no, default 1 | How far into the future to materialize. Also the length of one generation: a pattern block with no `cycles` stops at the first cycle boundary that covers this span, so it cannot book the channel years ahead. |
