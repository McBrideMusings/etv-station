...
3. **Embed program metadata.** Items carry title / description / season / episode / categories / rating / artwork — written into the `program` block of each playout item so ETV-next's XMLTV is populated. An episode's title is the series name and its own name goes in `sub_title`, and genre tags become categories, with no config; a `guide:` block cascading channel → block → pool → item (most specific field wins) authors any of these explicitly, with plain `{field}` templates and an `{a|b}` fallback (#158, #289). `<icon src>` is populated from artwork the station caches to disk at Plex ingest time and ETV-next serves from its own HTTP surface — never a Plex URL, since one carries a working access token (#187). Unset artwork caching (the default) means no artwork fetch and no `<icon>` element; an item whose fetch fails emits no `<icon>` and logs the item by name; re-ingesting an item whose Plex artwork changed refreshes the cached copy in place.
...
| `bucket_order` | — | A second sort deciding only the order the *series* come up in, when it should differ from the order their items play in |
...
A pattern step is `{pool, take, from, chance}`. `take` is a count, or the word `all` to empty whichever series the visit picked. `from` is where in that series the slice is cut — `start` (default, meaning where the series left off), `end` (the last `take` items), or `random` (a seeded offset, then `take` consecutive). `end` and `random` name absolute positions, so they ignore the resume cursor; that is what makes "the last three episodes" mean the same thing every visit. `chance` (default `1.0`) makes a step fire probabilistically — the "occasionally binge" knob. The roll is keyed on `(seed, cycle, step)`, so a pinned `seed` reproduces the whole skip/fire sequence, and a skipped step consumes no cursor.
...
**`show_id` is derived from the show's own GUIDs, not its title (#274).** The Plex ingester fetches each show's own external ids (`imdb`/`tmdb`/`tvdb`/`plex`, strongest first — the same first-hit-wins rule every other identity in this catalog uses) and, for a show with none, falls back to a hash of the show's Plex metadata key rather than its title — the same posture the `fs:` fallback takes for a GUID-less file. This is what `plex-db-ex` derives for the same show too, so a taste plugin holding one of this catalog's episodes can look its show up there directly. Before #274, `show_id` was `"show:" + the title verbatim`: two shows sharing a title (*The Office* UK and US both title `"The Office"`) collapsed into one series, and a Plex re-scrape that changed a title orphaned the show's resume position. **#274 shipped with a one-time reset:** every `show_id` this catalog held changed shape on that deploy, the title-keyed resume cursors it replaced stopped matching, and every show in every channel restarted from the top exactly once. This project is pre-release with one operator and no other consumers, so that reset was accepted rather than migrated — there is no dual-key read and none is planned.
...
**Two sorts, two questions.** `order` sorts the pool's flat list, and the series then rotate in order of first appearance in it — one answer to two questions, which holds only while both want the same sort. "Newest show first, but its episodes in episode order" does not: it wants `year:desc` between series and `season:asc,episode:asc` inside one. `bucket_order` is the second sort, read only for the series sequence, and a series is placed by whichever of its items that sort puts first — a rule checkable against the catalog by eye rather than an average of anything. It runs through the same ordering engine `order` uses, so a field cannot mean one thing in one and something else in the other.

Three combinations are refused at load rather than given a reading. `take: all` with `rotate: slot` has no single series to empty, since `slot` changes series every item. `take: all` with a `from` other than `start` moves a slice that `all` has already made the whole series. And `take: all` cannot share a block with `constraints`: converting a pool-draw gap into channel positions is arithmetic over the pattern's `take` numbers done at load, and how much a take-all step lays down is not known until its pool has been resolved — so the seam could only be sized by guessing, and a guess too small would let a repeat cross a generation boundary silently.
...
It also carries **checkpoints**: the scheduling state — pool rotation and list position — at every item boundary that has not aired yet. When the channel throws part of its written future away — a config edit or forced refresh cutting at the next item boundary, a coverage heal, the window sweep — it rewinds to the checkpoint where the surviving schedule ends and regenerates from there under the current config. So an edit reaches a pattern channel without waiting for its whole written window to play out, and a wipe that spares part of a generation repeats no item. One case can drop one: when an adjacency constraint moved an item later than one that followed it, and the wipe lands between the two, the moved item is treated as aired and skipped — a skip rather than a re-air, because the re-air is the one a viewer sees. Aired and currently-airing items are never touched.
...
- **Why a separate program (not a crate inside ETV-next)?** The station iterates on rules and metadata workflows unrelated to ETV-next's pipeline work, and keeping it its own program means it can be released on its own cadence as a standalone companion project. Vendoring ETV-next's source (below) does not change that — `crates/etv-station` and `vendor/etv-next` stay separate Cargo workspaces with separate concerns.
