...
([#411](https://github.com/McBrideMusings/etv-station/issues/411)). The
partition is not a preference — pools are blind to each other, so two movie
pools over one library have to be disjoint by construction or they will both air
...
**Shipped.** Per-channel `etv-overlay` subprocess renders Vello frames to a fifo etv-next composites on. Rhai scripts read the station-emitted chunked playout JSON to template lower-thirds with the current/next item's title, season, episode and year, and gate visibility on `item_elapsed` / `item_remaining`. Season and episode reach the script only for episodes, so `season >= 0` is what lets one script format a film as `Title (2025)` and an episode as `Show  S12E01` with no per-channel variant. Per-layer overrides (visibility, opacity, content, corner, and — new — `offset_x`/`offset_y`, a pixel offset added on top of `corner`/`margin` for a layer that animates its position, not just its visibility) compose with global `visible`/`opacity`. Sample scripts in `crates/etv-overlay/fixtures/scripts/`: `now_playing.rhai`, `up_next.rhai`, `pulse_watermark.rhai`, `corner_rotate.rhai`, `now_and_next.rhai`, `title_chyron.rhai` (slides the current item's title out from behind a corner logo on a configurable interval — live on the Pierce channels). The deployed station also runs a **Now/Next snipe** (`shared/now-next-snipe.rhai`) on all 60 channels carrying an inline overlay: two cards shown one after the other in the same bottom-left position — NOW naming the episode fully with its title on a second row, NEXT just the show name — with asymmetric easing, a staggered reveal and opacity tied to the travel.

Isolated overlay preview: `admin watch` (dev/prod → channel → live/overlay) or `admin overlay-watch <channel>` renders a channel's real overlay over a looping background fixture and streams it into VLC, hot-reloading on save — no station, ETV-next, or Plex needed. The background is solid black by default (`BG=bg-checkerboard-20s.mp4` for an alpha check instead), and `KIND=film|series` picks the stand-in program metadata, since nothing can infer a channel's kind with no station running. The cycle is shortened by overriding the script's own `interval_secs` (`INTERVAL`, default 12) rather than by scaling the clock — `--time-scale` compresses the animation along with the gap, turning an 8-second hold into an 0.8-second flash, so it now defaults to 1.
