...
**The pool can name the candidate queries itself (#210), and that is a refinement of the split rather than a reversal of it.** The paragraph under *Why* says the config author cannot be asked to write the expression that gathers a "For You" pool's candidates, and that stays true — but *which library* those candidates may come from is not a taste question at all. It is a fact about the station: this server has a Concerts section and a Power Hours section, each with a channel of its own, and neither belongs in a general recommendation feed. The script cannot answer that for one channel without answering it for every channel pointed at the same script, so asking the author to fork the script to exclude a library is asking them to fork a taste algorithm over a shelving decision.

So a pool may write a `sources:` table — set name to CEL expression — which replaces the script's `sources()` wholesale for that pool. The plugin still owns gathering in the sense this ADR means it: it runs whatever queries it is given, decides what is worth surfacing, and returns the set already ranked. What moved is the outer bound of the corpus, which is the kind of statement a config can make and a script cannot. Declining the field leaves the script's own table in force, which is what every channel written before #210 means.
