...
The pipeline says they are not the same thing. `resolve_channel` walks `config.rule.blocks` and concatenates each block's items into one flat list (`crates/etv-station/src/resolve.rs:156-192`). No wall-clock time exists at that point. Times are assigned afterwards by `Sequential`, which implements `Rule::items_covering(anchor_utc, from, to)` (`crates/etv-station/src/rule.rs:9-15`) by laying that flat list end to end. So:

- "Is today inside the Halloween window?" is answered once for a whole generation. Every deployed channel runs `window_days = 1`, so a generation is a day, and a date predicate consulted at `resolve.rs:156` is exact.
...
- **A date predicate on `BlockInclude`, evaluated before concatenation.** Non-matching dated blocks are dropped at `resolve.rs:156`; the rest concatenate exactly as now. Nothing downstream changes.
