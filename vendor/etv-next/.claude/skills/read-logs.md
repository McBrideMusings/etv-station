...
Admin commands write output to timestamped log files in `tmp/`. The file name pattern is:
...
Examples:
- `./admin dev` → `tmp/dev.2026-05-01_133442.log`
- `./admin build` → `tmp/build.2026-05-01_133442.log`
- `./admin test` → `tmp/test.2026-05-01_133442.log`
...
### Build problem (server didn't launch, panic on start)
...
### Runtime / behavior bug (server launched but something went wrong)
