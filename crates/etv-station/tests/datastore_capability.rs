...
//! `plexdb-reader` crate's four accessors through `ctx.datastore("name")`;
...
/// understands: one enrichment fact, one edge, and one play — just enough to
/// exercise every accessor's happy path with a value a script can assert on.
fn write_fixture(path: &std::path::Path) {
    ...
    conn.execute_batch(&format!(
        "CREATE TABLE schema_version (version INTEGER NOT NULL);
         ...
         INSERT INTO items (item_id, type) VALUES ('imdb:tt1', 'movie');
...
/// Calls all four accessors and throws (surfacing as a `pick()` error the
...
/// all four accessors and receives the real rows the fixture store holds.
#[test]
